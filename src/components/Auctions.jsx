    ...
    <div className="flex h-[360px] flex-col rounded-3xl border border-slate-100 bg-white p-5 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-[#021A34] text-xs font-bold text-white">07</div>
          <h2 className="font-heading text-base text-slate-800">{t.auctions}</h2>
        </div>

        <button type="button" onClick={onViewAll} className="font-heading text-xs text-[#344A61] hover:underline">
          {t.viewAll}
        </button>
      </div>
      ...
      {comment && (
        ...
        >
          <p className="font-heading text-[10px] uppercase tracking-[0.16em] text-[#344A61]">
            {comment.type}
          </p>
